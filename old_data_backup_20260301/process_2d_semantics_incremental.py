...
def process_drive(drive_dir: Path, cell_size_meters: float = 20.0) -> List[Dict]:
    """处理一个drive的置信度数据"""
    ...
    for cell_idx in tqdm(range(num_cells), desc=f"处理 {drive_dir.name}"):
        ...
        if len(cell_objects) >= 10:  # 至少10个对象
...
def generate_poses(cells: List[Dict], poses_per_cell: int = 50) -> List[Dict]:
    """为cells生成poses"""
    print("\n生成poses...")
    ...
    distances = [2, 3, 4, 5, 6, 7, 8, 10, 12, 15]
    ...
    for cell in tqdm(cells, desc="生成poses"):
        ...
        for i in range(poses_per_cell):
            ...
            radius = np.random.uniform(2, cell_size/2)
            ...
            if len(semantic_groups) >= 2:
                ...
                elif '{object}' in template and '{distance}' in template:
                    description = template.format(
                        object=obj_name1,
                        distance=np.random.choice(distances),
                        direction=np.random.choice(directions)
                    )
                elif '{object}' in template and '{direction}' in template:
                    description = template.format(
                        object=obj_name1,
                        direction=np.random.choice(directions)
                    )
                ...
                if '{object}' in template and '{distance}' in template:
                    description = template.format(
                        object=obj_name,
                        distance=np.random.choice(distances),
                        direction=np.random.choice(directions)
...
def main():
    print("="*80)
    print("增量处理 KITTI-360 2D语义置信度数据")
    print("="*80)
    
    data_dir = Path("/Users/yaoyingliang/Desktop/Text2Loc-main/data_2d_semantics/train")
    output_dir = Path("/Users/yaoyingliang/Desktop/Text2Loc-main/data/k360_2d_semantics_incremental")
    ...
    for drive in drives:
        ...
        print('='*60)
        cells = process_drive(drive, cell_size_meters=20.0)
    ...
    # 生成poses
    poses = generate_poses(all_cells, poses_per_cell=50)
    ...
    stats = {
        ...
        'drives_processed': [d.name for d in drives]
    }

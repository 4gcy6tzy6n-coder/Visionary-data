"""
处理KITTI360 3D语义数据 V3
正确处理static文件夹中的大型PLY文件
"""
...
class SemanticDataProcessorV3:
    """处理3D语义点云数据 - V3版本，专门处理static PLY"""
    ...
    def parse_static_ply(self, ply_file: Path, max_points: int = 500000) -> np.ndarray:
        """解析static PLY文件"""
        try:
            with open(ply_file, 'rb') as f:
                ...
                skip_ratio = num_vertices // read_vertices if num_vertices > max_points else 1
                
                # 数据格式: x,y,z (float32), r,g,b (uint8), semantic,int (int32), instance (int32), visible (uint8), confidence (float32)
                # 每个顶点: 3*4 + 3*1 + 2*4 + 1 + 4 = 12 + 3 + 8 + 1 + 4 = 28 bytes
                dtype = np.dtype([
                ...
                if skip_ratio > 1:
                    ...
                    for i in range(read_vertices):
                        ...
                        f.seek(header_size)
                        # 跳过前面的顶点
                        f.seek(i * 28, 1)  # 相对当前位置跳过
                        data = np.fromfile(f, dtype=dtype, count=1)
                        if len(data) > 0:
                            points_list.append([
                                data['x'][0], data['y'][0], data['z'][0], data['semantic'][0]
                            ])
                    points = np.array(points_list) if points_list else np.array([])
                else:
                    # 直接读取所有
    ...
    def extract_objects_from_points(self, points: np.ndarray) -> List[Dict]:
        ...
        for label_id in unique_labels:
            ...
            # 过滤异常物体
            if np.any(size > 100) or np.any(np.isnan(center)):
                continue
    ...
    def process_scene(self, scene_dir: Path) -> List[Dict]:
        ...
        if not static_dir.exists():
            logger.warning(f"  {scene_name}: static目录不存在")
            return []
        ...
        # 只处理前10个文件（避免内存溢出）
        all_objects = []
        for ply_file in tqdm(ply_files[:10], desc=f"  处理 {scene_name}"):
            points = self.parse_static_ply(ply_file, max_points=100000)
            if len(points) > 0:
    ...
    def create_cells(self, scenes_data: Dict[str, List[Dict]], cell_size: float = 10.0) -> List[Dict]:
        ...
        for scene_name, objects in scenes_data.items():
            ...
            num_x = min(30, max(1, int(range_x / cell_size) + 1))
            num_y = min(30, max(1, int(range_y / cell_size) + 1))
...
def main():
    ...
    output_path = "/Users/yaoyingliang/Desktop/Text2Loc-main/data/k360_from_semantics_v3"
    
    logger.info("\n" + "="*60)
    logger.info("处理KITTI360 3D语义数据 V3")
    logger.info("="*60)
    
    processor = SemanticDataProcessorV3(data_path)

...
class Text2LocDataset(Dataset):
    ...
    def _compute_normalization_stats(self):
        ...
        logger.info(f"📐 坐标归一化: mean={self.global_mean}, std={self.global_std}")
    ...
    def _denormalize(self, xy: np.ndarray) -> np.ndarray:
        """反归一化坐标"""
        return xy * self.global_std + self.global_mean
    ...
    def __getitem__(self, idx):
        ...
        for obj in objects:
            if isinstance(obj, dict):
                ...
                feat = [
                    obj_offset[0],  # 相对于cell中心的X偏移
                    obj_offset[1],  # 相对于cell中心的Y偏移
                    float(center[2]) if len(center) > 2 else 0,  # Z保持绝对值
                    0.5, 0.5, 0.5  # 默认颜色
                ]
...
def train_model(data_path: str = None, num_epochs: int = 50, batch_size: int = 32, lr: float = 0.001):
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_error < best_val_error:
            ...
            }, save_path / 'normalized_best_model.pth')
    ...
    return model, train_losses, val_errors, best_val_error


if __name__ == "__main__":
    model, train_losses, val_errors, best_error = train_model(
        num_epochs=50,
        batch_size=32,
        lr=0.001
    )

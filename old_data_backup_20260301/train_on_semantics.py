...
def train_model(data_path: str, num_epochs: int = 100, batch_size: int = 16, lr: float = 0.001):
    """训练模型"""
    
    data_path = Path(data_path)
    
    logger.info("\n" + "="*60)
    logger.info("加载3D语义数据集")
    logger.info("="*60)
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_error < best_val_error:
            ...
            }, save_path / 'semantics_best_model.pth')
...
if __name__ == "__main__":
    model, best_error = train_model(
        data_path="/Users/yaoyingliang/Desktop/Text2Loc-main/data/k360_from_semantics_v3",
        num_epochs=100,
        batch_size=16,
        lr=0.001
    )
